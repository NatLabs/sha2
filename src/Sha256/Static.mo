...
module Sha256 {
  ...
  public func algo(state: StaticSha256) : Algorithm = state.0;
  ...
  public func reset(state: StaticSha256) {
    ...
    if (state.0 == #sha224){
      state.7[0] := 0xc105; state.8[0] := 0x9ed8;
      state.7[1] := 0x367c; state.8[1] := 0xd507;
      state.7[2] := 0x3070; state.8[2] := 0xdd17;
      state.7[3] := 0xf70e; state.8[3] := 0x5939;
      state.7[4] := 0xffc0; state.8[4] := 0x0b31;
      state.7[5] := 0x6858; state.8[5] := 0x1511;
      state.7[6] := 0x64f9; state.8[6] := 0x8fa7;
      state.7[7] := 0xbefa; state.8[7] := 0x4fa4;
    } else {
      state.7[0] := 0x6a09; state.8[0] := 0xe667;
      state.7[1] := 0xbb67; state.8[1] := 0xae85;
      state.7[2] := 0x3c6e; state.8[2] := 0xf372;
      state.7[3] := 0xa54f; state.8[3] := 0xf53a;
      state.7[4] := 0x510e; state.8[4] := 0x527f;
      state.7[5] := 0x9b05; state.8[5] := 0x688c;
      state.7[6] := 0x1f83; state.8[6] := 0xd9ab;
      state.7[7] := 0x5be0; state.8[7] := 0xcd19;
    };
  };

  public func writeByte(state: StaticSha256, val: Nat8){
    if (state.5[0]){
      state.6[0] := nat8To16(val) << 8;
      state.5[0] := false;
    } else {
      state.1[Nat8.toNat(state.3[0])] := state.6[0] | nat8To16(val);
      state.3[0] +%= 1;
      state.5[0] := true;
    };
  ...
  public func writePadding(state: StaticSha256): (){
    ...
    let t : Nat8 = if (state.5[0]) state.3[0] << 1 else state.3[0] << 1 +% 1;
    let n_bits : Nat64 = ((nat32To64(state.4[0]) << 6) +% Nat64.fromIntWrap(Nat8.toNat(t))) << 3;
    // separator byte
    if (state.5[0]) {
      state.1[Nat8.toNat(state.3[0])] := 0x8000;
    } else {
      state.1[Nat8.toNat(state.3[0])] := state.6[0] | 0x80;
    };
    state.3[0] +%= 1;
    // zero padding with extra block if necessary
    if (state.3[0] > 28) {
      while (state.3[0] < 32) {
        state.1[Nat8.toNat(state.3[0])] := 0;
        state.3[0] +%= 1;
      };
      process_block(state);
      state.3[0] := 0;
      // skipping here: state.4[0] +%= 1;
    };
    ...
    state.1[28] := nat32To16(lh >> 16);
    state.1[29] := nat32To16(lh & 0xffff);
    state.1[30] := nat32To16(ll >> 16);
    state.1[31] := nat32To16(ll & 0xffff);
    process_block(state);
    // skipping here: state.3[0] := 0;
  };
  ...
  public func sum(state: StaticSha256): Blob {
    ...
    state.2[0] := nat16To8(state.7[0] >> 8);
    state.2[1] := nat16To8(state.7[0] & 0xff);
    state.2[2] := nat16To8(state.8[0] >> 8);
    state.2[3] := nat16To8(state.8[0] & 0xff);
    state.2[4] := nat16To8(state.7[1] >> 8);
    state.2[5] := nat16To8(state.7[1] & 0xff);
    state.2[6] := nat16To8(state.8[1] >> 8);
    state.2[7] := nat16To8(state.8[1] & 0xff);
    state.2[8] := nat16To8(state.7[2] >> 8);
    state.2[9] := nat16To8(state.7[2] & 0xff);
    state.2[10] := nat16To8(state.8[2] >> 8);
    state.2[11] := nat16To8(state.8[2] & 0xff);
    state.2[12] := nat16To8(state.7[3] >> 8);
    state.2[13] := nat16To8(state.7[3] & 0xff);
    state.2[14] := nat16To8(state.8[3] >> 8);
    state.2[15] := nat16To8(state.8[3] & 0xff);
    state.2[16] := nat16To8(state.7[4] >> 8);
    state.2[17] := nat16To8(state.7[4] & 0xff);
    state.2[18] := nat16To8(state.8[4] >> 8);
    state.2[19] := nat16To8(state.8[4] & 0xff);
    state.2[20] := nat16To8(state.7[5] >> 8);
    state.2[21] := nat16To8(state.7[5] & 0xff);
    state.2[22] := nat16To8(state.8[5] >> 8);
    state.2[23] := nat16To8(state.8[5] & 0xff);
    state.2[24] := nat16To8(state.7[6] >> 8);
    state.2[25] := nat16To8(state.7[6] & 0xff);
    state.2[26] := nat16To8(state.8[6] >> 8);
    state.2[27] := nat16To8(state.8[6] & 0xff);

    if (state.0 == #sha224) return Blob.fromArrayMut(state.2);

    state.2[28] := nat16To8(state.7[7] >> 8);
    state.2[29] := nat16To8(state.7[7] & 0xff);
    state.2[30] := nat16To8(state.8[7] >> 8);
    state.2[31] := nat16To8(state.8[7] & 0xff);

    return Blob.fromArrayMut(state.2);
  };

  func process_block(state: StaticSha256) {

      let w00 = nat16To32(state.1[0]) << 16 | nat16To32(state.1[1]);
      let w01 = nat16To32(state.1[2]) << 16 | nat16To32(state.1[3]);
      let w02 = nat16To32(state.1[4]) << 16 | nat16To32(state.1[5]);
      let w03 = nat16To32(state.1[6]) << 16 | nat16To32(state.1[7]);
      let w04 = nat16To32(state.1[8]) << 16 | nat16To32(state.1[9]);
      let w05 = nat16To32(state.1[10]) << 16 | nat16To32(state.1[11]);
      let w06 = nat16To32(state.1[12]) << 16 | nat16To32(state.1[13]);
      let w07 = nat16To32(state.1[14]) << 16 | nat16To32(state.1[15]);
      let w08 = nat16To32(state.1[16]) << 16 | nat16To32(state.1[17]);
      let w09 = nat16To32(state.1[18]) << 16 | nat16To32(state.1[19]);
      let w10 = nat16To32(state.1[20]) << 16 | nat16To32(state.1[21]);
      let w11 = nat16To32(state.1[22]) << 16 | nat16To32(state.1[23]);
      let w12 = nat16To32(state.1[24]) << 16 | nat16To32(state.1[25]);
      let w13 = nat16To32(state.1[26]) << 16 | nat16To32(state.1[27]);
      let w14 = nat16To32(state.1[28]) << 16 | nat16To32(state.1[29]);
      let w15 = nat16To32(state.1[30]) << 16 | nat16To32(state.1[31]);
      ...
      //   let (v0, v1) = (state.1[j], state.1[l]);
      //   let s0 = rot(v0, 07) ^ rot(v0, 18) ^ (v0 >> 03);
      //   let s1 = rot(v1, 17) ^ rot(v1, 19) ^ (v1 >> 10);
      //   state.1[m] := msg[i] +% s0 +% msg[k] +% s1;
      // };
      
      // compress
      let a_0 = nat16To32(state.7[0]) << 16 | nat16To32(state.8[0]);
      let b_0 = nat16To32(state.7[1]) << 16 | nat16To32(state.8[1]);
      let c_0 = nat16To32(state.7[2]) << 16 | nat16To32(state.8[2]);
      let d_0 = nat16To32(state.7[3]) << 16 | nat16To32(state.8[3]);
      let e_0 = nat16To32(state.7[4]) << 16 | nat16To32(state.8[4]);
      let f_0 = nat16To32(state.7[5]) << 16 | nat16To32(state.8[5]);
      let g_0 = nat16To32(state.7[6]) << 16 | nat16To32(state.8[6]);
      let h_0 = nat16To32(state.7[7]) << 16 | nat16To32(state.8[7]);
      var a = a_0;
      var b = b_0;
      var c = c_0;
      var d = d_0;
      ...
      //   let t = h +% K[i] +% state.1[i] +% ch +% sigma1;
      //   h := g;
      //   g := f;
      //   f := e;
      //   e := d +% t;
      ...
      g +%= g_0;
      h +%= h_0;
      state.7[0] := nat32To16(a >> 16); state.8[0] := nat32To16(a & 0xffff);
      state.7[1] := nat32To16(b >> 16); state.8[1] := nat32To16(b & 0xffff);
      state.7[2] := nat32To16(c >> 16); state.8[2] := nat32To16(c & 0xffff);
      state.7[3] := nat32To16(d >> 16); state.8[3] := nat32To16(d & 0xffff);
      state.7[4] := nat32To16(e >> 16); state.8[4] := nat32To16(e & 0xffff);
      state.7[5] := nat32To16(f >> 16); state.8[5] := nat32To16(f & 0xffff);
      state.7[6] := nat32To16(g >> 16); state.8[6] := nat32To16(g & 0xffff);
      state.7[7] := nat32To16(h >> 16); state.8[7] := nat32To16(h & 0xffff);
